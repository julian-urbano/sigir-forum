...
SIGIR Forum editors\\
\small{\href{mailto:editors_SIGIR@acm.org}{\color{black}\nolinkurl{editors_SIGIR@acm.org}}}
}
\maketitle 
\begin{abstract}
This document contains the instructions for the SIGIR Forum publication template in \LaTeX\xspace as of version {{SIGIRFORUMVERSION}}, {{SIGIRFORUMDATE}}. For the latest version, please visit the Github repository.\footnote{\url{https://github.com/julian-urbano/sigir-forum/releases/latest}}
\end{abstract}
...
\begin{verbatim}
    \authors{
        \author[jane_doe@sigir.org]{Jane Doe}{ACM SIGIR}{USA}
        \and
...
Please refrain from using \href{https://sigir.org}{hidden links like this}. All in-text URLs must be written with the \texttt{\textbackslash url} command, and they should appear in footnotes whenever possible. Exceptions are possible if adding footnotes would harm readability; please contact the editors if unsure. If you want to provide an email address in the text, use command \texttt{\textbackslash email}.
...
You can submit your publication via email to \email{editors_SIGIR@acm.org}. Please include all the source files and a compiled PDF.

Attached to these instructions there is a \textbf{checklist document}\footnote{\url{https://github.com/julian-urbano/sigir-forum/releases/download/v{{SIGIRFORUMVERSION}}/checklist.pdf}} with a list of items to check that your manuscript follows the SIGIR Forum template. Please check all items before you make a submission.
